# SSCCE for conflicting rule fixes

This is a demonstration of a problem that occurs in linters (ESLint for JavaScript in this SSCCE, but it's not the only one) where the linter will fix multiple issues concurrently, and in doing so introduces a problem that would not have been introduced if fixes were only applied after a re-analysis of the code.

An accompanying article can be found [on my blog](https://jfmengels.net/concurrent-linter-fixes/). *(There are minute differences but the essence is very much the same)*

## SSCCE

[`example.js`](./example.js) contains the code we want to analyze.
...
This [SSCCE](https://sscce.org/) tries to demonstrate this problem, and does so by creating and enabling 2 linter rules:
